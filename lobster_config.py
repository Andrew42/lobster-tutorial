...
# IMPORTANT NOTE: Due to a bug in the way dependent workflow tasks update their expected number of
#   remaining units to process, the 'files_per_task' of the parent dataset must be set to 1. Otherwise
#   the child workflow will get stuck in an endless loop of failing to create tasks due to insufficient
#   units from the parent dataset.
dataset=Dataset(
